...
class GenerateBlogImages extends Command
{
    ...
    {
        $settings = rescue(fn () => app(\App\Settings\BlogSettings::class), null, false);

        if ($settings && ! $settings->ai_images_enabled) {
            $this->info('AI cover images are switched OFF in Admin → Blog & AI Images.');

            return self::SUCCESS;
        }

        // Only posts that are actually LIVE (published status + time reached):
        // scheduled posts get their cover generated by the daily cron at the
        // moment they go live — one fresh image per publish, exactly when the
        // reader can first see it.
        ...
        foreach ($posts as $post) {
            ...
            try {
                // Provider priority is owner-controlled (Admin → Blog & AI
                // Images): Gemini first with a daily cap, Pollinations
                // (keyless, free) as the always-available fallback.
                $binary = (($settings->image_provider ?? 'gemini_first') === 'pollinations_only'
                        ? null
                        : $this->generateViaGemini($prompt))
    ...
    private function generateViaGemini(string $prompt): ?string
    {
        ...
        // Owner-set daily cap (Admin → Blog & AI Images): beyond it, the free
        // fallback takes over — protects the free allowance now and the bill
        // later once Gemini billing is enabled.
        $limit = (int) (rescue(fn () => app(\App\Settings\BlogSettings::class)->gemini_daily_limit, 2, false));
        $cacheKey = 'gemini-covers-'.now('Asia/Karachi')->format('Ymd');

        if (\Illuminate\Support\Facades\Cache::get($cacheKey, 0) >= $limit) {
            $this->line('  gemini: daily cap ('.$limit.') reached — using fallback');
            ...
            foreach (($response->json('candidates.0.content.parts') ?? []) as $part) {
                ...
                if ($data !== null) {
                    ...
                    if ($binary !== false && strlen($binary) > 10_000) {
                        ...
                        \Illuminate\Support\Facades\Cache::put($cacheKey, \Illuminate\Support\Facades\Cache::get($cacheKey), now()->addDay());
